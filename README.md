...
* 일반 예외 발생시 `Call Trace`가 표시되는 대신 `제가 뭐라고 했습니까?`라는 안보 경고가 표시됩니다.
* 안보 예외 발생시 `주문. 피청구인 대통령 박근혜를 파면한다.`라는 안보 경고가 표시됩니다.
...
~~~
### 익명함수
* `익명함수`는사용할 수 없습니다. 익명성은 대한민국의 안보를 안전치 못하게 합니다. 
* `익명함수`를 강제로 사용할 시,`안보 경고`가 표시됩니다
익명함수는, 아래와 같은 방식으로 정의합니다.
~~~
공천 <변수이름> 는 이것이다 전부 이렇게 해 가지고 으로 들어가도록(매개변수){
 /ㄹ 함수 내용을 여기에 작성하셔야 한다라고 생각합니다 ㄹ/
}
~~~
익명함수 호출 방법은 다른 프로그래밍 언어들과 거의 비슷하다고 생각한다
~~~
<변수이름>(매개변수);
~~~
함수 호출 방법은 다른 프로그래밍 언어들과 거의 비슷하다고 생각한다
